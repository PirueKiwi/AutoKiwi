...
using AutoKiwi.ContextManagement; // Add this using directive
using PromptType = AutoKiwi.Translation.PromptType; // Add this alias at the top of the file

// Ensure the ContextManager class is defined in the AutoKiwi.ContextManagement namespace
    ...
    public class MultiPassCodeGenerator : ICodeGenerator
    {
            ...
            ApplicationType applicationType)
        {
            ...
            try
            {
                ...
                await GenerateImplementationPlansAsync(components, conversationId);
            ...
            string conversationId)
        {
            ...
            await _contextManager.AddMessageAsync(
                conversationId,
                new TranslationConversationMessage
                {
                    Role = "assistant",
        ...
        /// <summary>
        /// Pátý pass - Odhalení vzájemných závislostí mezi komponenty
        /// </summary>
        private async Task<List<ApplicationComponent>> DetectComponentDependenciesAsync(
            List<ApplicationComponent> components,
            string conversationId)
        {
            LogMessage("Pass 5: Odhalení vzájemných závislostí mezi komponenty");

            foreach (var component in components)
            {
                // Simulate dependency detection logic
                component.Dependencies = components
                    .Where(c => c != component && c.Name.Contains("Dependency"))
                    .Select(c => c.Name)
                    .ToList();
            }

            // Publish an event about updated dependencies
            _eventBus.Publish(new DependenciesUpdatedEvent(conversationId, components));
...
namespace GeneratedApp
{
    public class MainForm : Form
    {
        ...
        /// <summary>
        /// Sedmý pass - Finální kontrola a čištění kódu
        /// </summary>
        private async Task<string> PolishFinalCodeAsync(
            string integratedCode,
            string applicationDescription,
            ApplicationType applicationType,
            string conversationId)
        {
            LogMessage("Pass 7: Finální kontrola a čištění kódu");

            // Vytvoření kontextu pro prompt
            var workflowContext = new WorkflowContext
            {
                Description = applicationDescription,
                CurrentCode = integratedCode,
                OperationId = conversationId,
                CurrentStage = WorkflowStage.FinalPolishing
            };

            // Tvorba promptu pro finální úpravy
            string prompt = await _promptTranslator.TranslateToModelPromptAsync(
                $"Polish and finalize this C# code for a {applicationType} application:\n\n{integratedCode}",
                "codellama:13b",
                PromptType.CodePolishing,
                workflowContext);
            ...
            // Extrakce kódu z odpovědi
            string polishedCode = ExtractCode(response);

            // Ověření, že vylepšený kód není prázdný
            if (string.IsNullOrWhiteSpace(polishedCode))
            {
                LogMessage("Vylepšený kód je prázdný, vracím původní kód", MessageSeverity.Warning);
                return integratedCode;
            }

            LogMessage("Finální kontrola a čištění kódu dokončeno");
            return polishedCode;
        }
    }
