...
namespace AutoKiwi.Orchestration
{
    ...
    public class AdaptiveSelector<T>
    {
        ...
        public async Task<T> SelectAsync(string task, Func<string, double> contextSimilarity)
        {
            ...
            foreach (var option in options)
            {
                ...
                double successScore = option.Value.Rate;
        ...
        private void UpdateSuccessRate(string key, bool success, double confidenceLevel)
        {
            ...
            {
                // Zvýšení úspěšnosti
                rate.Rate = rate.Rate * (1 - learningFactor) + 1.0 * learningFactor;
            }
            else
            {
                // Snížení úspěšnosti
                rate.Rate = rate.Rate * (1 - learningFactor) + 0.0 * learningFactor;
            }
        ...
        private void ApplyDecay(string key)
        {
            ...
            rate.Rate = rate.Rate * _decayRate; // Corrected property name from 'SuccessRate' to 'Rate'
        }
            ...
            foreach (var entry in _successRates)
            {
                if (entry.Key.StartsWith(task + ":"))
                {
                    ...
                    statistics.Add(new SuccessStatistics
                    {
                        TaskName = task,
                        OptionId = optionId,
                        SuccessRate = rate.Rate, // Corrected property name from 'SuccessRate' to 'Rate'
        ...
        public void RegisterOption(string task, T option, double initialSuccessRate = 0.5)
        {
            ...
            if (!_successRates.ContainsKey(key))
            {
                _successRates[key] = new SuccessRate
                {
                    Rate = initialSuccessRate, // Corrected property name from 'SuccessRate' to 'Rate'
                    TotalAttempts = 0,
    ...
    public class AdaptiveLearningModel
    {
        ...
        {
            // Removed the check for evt.ModelId as it does not exist in CompilationCompletedEvent
